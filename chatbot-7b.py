...
endpoint_name = "falcon-7b-instruct-4xl"
...
class ContentHandler(LLMContentHandler):
    ...
    def transform_input(self, prompt: str, model_kwargs: Dict) -> bytes:
        ...
        input_str = json.dumps({"inputs": prompt, "parameters": {"stop": ["Human:"], "do_sample": False, "repetition_penalty": 1.1}})
...
with container:
    ...
    # when a file is uploaded we also send the content to the chatchain object and ask for confirmation
        ...
        st.session_state['past'].append("I have uploaded a file. Please confirm that you have read it.")
        st.session_state['generated'].append("Ok, I'm ready!")
        chatchain.memory.save_context({"input": "Please read the following document and answer questions based on its content\n" + stringio.read().strip()}, {"output": "Ok"})
    ...
    #st.write(f"Conversation history: {history}")
